  ...
  return {
    ...
    visitor: {
      ImportDeclaration(path) {
        if (path.node.source.value !== 'rax' || path.node.source.type !== "StringLiteral") {
          return;
        }
        const REACT_METHODS = ['createElement', 'Component']
            ...
            importSpecifierPath.traverse({
              Identifier(identifierPath) {
                ...
                if (REACT_DOM_METHODS.includes(methodName)) {
                  ...
                  reactMethods.add(methodName)
                  console.warn(`当前方法 ${methodName} 没有进行配置，直接从React上获取，如有问题请检查此方法。`)
                }
              }
            })
          }
        ...
        const importReactDOMTemplate = template.ast(`
              import { ${Array.from(reactDOMMethods).join(',')}  } from 'react-dom';
            `);
